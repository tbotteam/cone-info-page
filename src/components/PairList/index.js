  ...
  const ListItem = ({ pairAddress, index }) => {
    ...
    if (pairData && pairData.token0 && pairData.token1) {
      ...
      const apy = formattedPercent(
        ((pairData.oneDayVolumeUSD ? pairData.oneDayVolumeUSD : pairData.oneDayVolumeUntracked) * 0.001 * 365 * 100) /
    ...
    Object.keys(pairs)
      ...
      .sort((addressA, addressB) => {
        ...
        if (sortedColumn === SORT_FIELD.APY) {
          const apy0 = parseFloat(pairA.oneDayVolumeUSD * 0.001 * 356 * 100) / parseFloat(pairA.reserveUSD)
          const apy1 = parseFloat(pairB.oneDayVolumeUSD * 0.001 * 356 * 100) / parseFloat(pairB.reserveUSD)
